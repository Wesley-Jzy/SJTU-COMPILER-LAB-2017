...
G_nodeList G_subNodeList(G_nodeList a, G_nodeList b) {
  G_nodeList res = NULL;
  for (G_nodeList nodes = a; nodes; nodes = nodes->tail) {
    if (!G_inNodeList(nodes->head, b)) {
...
G_nodeList G_unionNodeList(G_nodeList a, G_nodeList b) {
  G_nodeList res = a;
  for (G_nodeList nodes = b; nodes; nodes = nodes->tail) {
    if (!G_inNodeList(nodes->head, a)) {
      res = G_NodeList(nodes->head, res);
    }
  }
